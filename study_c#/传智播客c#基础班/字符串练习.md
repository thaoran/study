...
for (int i = 0; i < contents.Length; i++)
{
    ...
    Console.WriteLine((strNew[0].Length > 10 ? strNew[0].Substring(0, 8) + "......" : strNew[0]) + "|" + strNew[1]);//三元表达式
}
Console.ReadKey();
//历史就是这么回事儿|袁腾飞
...
//找单个或
string str="adaefgeegsegesegsgeg"
int count = 1;//用来记录e出现的次数
while (index != -1)
{
   count++;
   index = str.IndexOf('e', index + 1);//每一次都是从上次找到的e之后一个开始
   if (index == -1)//没找到就不再打印
   {
       break;
   }
   Console.WriteLine("第{0}次出现e的位置是{1}",count,index);//找不到就输出-1
}
Console.ReadKey();
